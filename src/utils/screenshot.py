...
class ScreenshotTool:
    ...
    async def _wait_for_response_complete(self, page: Page, platform_id: str) -> bool:
        ...
        try:
            logger.info(f"{platform_id}正在等待AI回复完成...")
                ...
                try:
                    ...
                    if textarea:
                        ...
                        if is_disabled is None:
                            logger.info(f"{platform_id}输入框已可用，回复可能已完成")
            ...
            logger.info(f"{platform_id}等待回复完成超时（15秒），但继续尝试截图")
            return True
        except Exception as e:
            logger.warning(f"{platform_id}等待回复完成时出错：{str(e)}")
            return True
    ...
    async def _deepseek_download_shared_image(self, page: Page, question: Question) -> Optional[str]:
        ...
        try:
            ...
            create_link_selectors = [
                "button.ds-atom-button.ds-basic-button.ds-basic-button--primary:has-text('创建并复制')",
                "button[role='button']:has-text('创建并复制')",
                ".ds-basic-button--primary:has-text('创建并复制')",
                        ...
                        for elem in elements:
                            try:
                                ...
                                if is_visible:
                                    ...
                                    logger.info(f"{platform_id}【第三步】找到创建分享链接按钮: {selector}")
                                    break
                            except:
                                continue
            ...
            logger.info(f"{platform_id}【第三步】点击创建并复制按钮...")
            await create_link_button.click()
            await page.wait_for_timeout(2000)  # 等待复制操作完成
            ...
            try:
                ...
                if shared_link:
                    ...
                    with open(link_path, 'w', encoding='utf-8') as f:
                        f.write(shared_link)
                    logger.info(f"{platform_id}【第四步】分享链接已保存：{link_path}")
                    logger.info(f"{platform_id}【第四步】分享链接内容: {shared_link}")
                else:
                    logger.info(f"{platform_id}【第四步】剪贴板为空")
            except Exception as e:
                logger.info(f"{platform_id}【第四步】获取剪贴板失败: {str(e)}")
