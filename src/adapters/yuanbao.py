...
class YuanbaoAdapter(BaseAdapter):
    ...
    async def _execute_login(self) -> bool:
        try:
            ...
            if submit_selector:
                ...
                logger.info("元宝登录表单已提交")
            else:
                logger.warning("元宝未找到提交按钮")
            
            return True
        except Exception as e:
            logger.error(f"元宝自动登录失败：{str(e)}")
            return False
    ...
    async def _send_message(self, question: str) -> None:
        try:
            ...
            logger.info(f"元宝成功发送消息：{question[:30]}...")
