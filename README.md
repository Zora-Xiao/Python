...
### v2.4.0 (2026-07-25)
- ✨ DeepSeek平台登录流程优化：删除重写的Cookie方法，使用基类标准流程，实现自动登录+Cookie保存+失败时手动重新登录
- ✨ Qwen平台登录流程优化：添加登录成功后保存Cookie步骤，确保自动登录+Cookie保存+失败时手动重新登录流程正确
- ✨ 豆包平台登录模式调整：改为手动登录模式，Cookie验证失败时自动等待手动重新登录
- ✨ 元宝平台登录模式调整：改为手动登录模式，Cookie验证失败时自动等待手动重新登录
- ✨ 文心一言平台登录模式调整：改为手动登录模式，Cookie验证失败时自动等待手动重新登录
- 📝 更新 README.md：修正各平台登录策略说明，明确DeepSeek/Qwen支持自动登录，其他平台需要手动登录
